...
def test_column_transformer_remainder(backend):
    ...
    ct = ColumnTransformerNoStack(
        [("name", "passthrough", slice(0, 0))], remainder="passthrough"
    )
    Xt = ct.fit_transform(X)
    assert len(Xt) == 2
...
def test_make_column_transformer(backend):
    ...
    trans = StandardScaler()
    ct = make_column_transformer_no_stack((trans, slice(0, 3)), ("passthrough", [3, 4]))
...
def test_column_transformer_in_pipeline(backend):
    ...
    pipe = make_pipeline(
        ...
        GroupRidgeCV(
            groups="input",
            solver_params=dict(n_iter=np.ones((1, 2)), progress_bar=False),
        ),
    )
    pipe.fit(X, Y)
