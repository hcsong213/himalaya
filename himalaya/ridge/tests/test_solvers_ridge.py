...
def _create_dataset(backend, many_targets=False):
    ...
    weights = backend.asarray(backend.randn(n_features, n_targets), backend.float64)
...
def test_solve_kernel_ridge(solver_name, backend, many_targets):
    ...
    for alpha in alphas:
        ...
        for ii in range(n_targets):
            ...
            b1 = scipy.linalg.solve(
                backend.to_numpy(XTX + reg), backend.to_numpy(XTY[:, ii])
            )
            ...
            assert_array_almost_equal(prediction, prediction_sklearn, decimal=6)
...
def test_warning_kernel_ridge_ridge(solver_name, backend):
    ...
    with pytest.warns(UserWarning, match="ridge is slower than solving kernel"):
        solver(X[:4], Y[:4])
