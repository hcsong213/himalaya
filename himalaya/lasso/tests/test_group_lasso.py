...
def test_group_lasso_decreasing(backend):
    ...
    coef, losses = solve_sparse_group_lasso(
        X, Y, max_iter=500, tol=1e-8, progress_bar=False, debug=True, momentum=False
    )
...
def test_group_lasso_vs_lasso(backend, n_targets_batch):
    ...
    for l1_reg in backend.logspace(-5, 5, 5):
        ...
        ols = sklearn.linear_model.Lasso(
            fit_intercept=False, alpha=float(l1_reg), max_iter=1000, tol=1e-8
        ).fit(backend.to_numpy(X), backend.to_numpy(Y))
...
def test_group_lasso_regularization_per_target(backend, n_targets_batch):
    ...
    for tt in range(n_targets):
        ...
        coef_tt = solve_sparse_group_lasso(
            X,
            Y[:, tt : tt + 1],
            groups=None,
            l21_reg=l21_reg[tt],
...
def test_group_lasso_cv(backend):
    ...
    coef, best_l21_reg, best_l1_reg, all_cv_scores = solve_sparse_group_lasso_cv(
        X,
        Y,
        cv=2,
        groups=None,
