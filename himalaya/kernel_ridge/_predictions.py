...
def predict_weighted_kernel_ridge(
    Ks,
    dual_weights,
    deltas,
    split=False,
    ...
    if split:
        Y_hat_full = backend.zeros_like(deltas, shape=(n_kernels, n_samples, n_targets))
    else:
    ...
    for start in bar(
        list(range(0, n_targets, n_targets_batch)), title="predict", use_it=progress_bar
    ):
        ...
        if split:
            if intercept is not None:
                raise ValueError("Cannot split the predictions with an intercept.")
...
def predict_and_score_weighted_kernel_ridge(
    Ks,
    dual_weights,
    deltas,
    Y,
    ...
    if n_targets_batch is None:
        n_targets_batch = n_targets
    for start in bar(
        list(range(0, n_targets, n_targets_batch)),
        title="predict_and_score",
        use_it=progress_bar,
    ):
        batch = slice(start, start + n_targets_batch)
        predictions = predict_weighted_kernel_ridge(
            Ks,
            dual_weights[:, batch],
            _batch_or_skip(deltas, batch, axis=1),
            split=split,
        )
        if intercept is not None:
            score_batch = score_func(Y[:, batch] - intercept[None, batch], predictions)
        else:
