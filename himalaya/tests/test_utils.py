...
_parameters = {
    ...
    "params_2": dict(
        ...
        n_samples_train=90,
        n_samples_test=40,
        kernel_weights=np.random.rand(40, 3),
...
def test_generate_multikernel_dataset(backend, name):
    ...
    (X_train, X_test, Y_train, Y_test, kernel_weights, n_features_list) = (
        generate_multikernel_dataset(**kwargs)
    )
    ...
    if kwargs["kernel_weights"] is not None:
        assert_array_almost_equal(kwargs["kernel_weights"], kernel_weights)
