...
def _create_data(backend, dtype_str):
    ...
    y_pred = backend.asarray(
        backend.randn(n_alphas, n_samples, n_targets), dtype=getattr(backend, dtype_str)
    )
    y_true = backend.asarray(
        backend.randn(n_samples, n_targets), dtype=getattr(backend, dtype_str)
    )
